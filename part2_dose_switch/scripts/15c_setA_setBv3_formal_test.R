...
df <- read.csv("~/alu_brca/part2_dose_switch/results/macs_real/nearest_alu_setA_setBv3_combined.csv")
...
subfam_counts$ratio_B_over_A <- round(subfam_counts$SetB_v3_ER_H3K27ac_PROseq / subfam_counts$SetA_ER_H3K27ac, 3)
...
total_B <- sum(subfam_counts_full$SetB_v3_ER_H3K27ac_PROseq)

subfam_counts_full$pval <- apply(subfam_counts_full, 1, function(row) {
  a <- as.numeric(row["SetA_ER_H3K27ac"]); b <- as.numeric(row["SetB_v3_ER_H3K27ac_PROseq"])
...
subfam_counts_full$ratio <- round(subfam_counts_full$SetB_v3_ER_H3K27ac_PROseq / subfam_counts_full$SetA_ER_H3K27ac, 3)
