...
pM <- read_fimo_fixed("fimo_results/pM_star_v2", 2706) %>% mutate(group = "pM_star")
nM <- read_fimo_fixed("fimo_results/nM_star_v2", 18893) %>% mutate(group = "nM_star")
combined <- bind_rows(pM, nM)
print(combined)

cat("\n--- Fisher's exact test per motif (fixed p<0.0001) ---\n")
