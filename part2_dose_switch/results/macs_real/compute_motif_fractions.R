...
read_fimo <- function(path, n_total) {
  df <- read.delim(file.path(path, "fimo.tsv"), comment.char = "#")
  df <- df[!is.na(df$motif_id), ]  # drop trailing summary lines
...
pM <- read_fimo("fimo_results/pM_star_v2", 2706) %>% mutate(group = "pM_star")
nM <- read_fimo("fimo_results/nM_star_v2", 18893) %>% mutate(group = "nM_star")
...
write.csv(combined, "motif_fractions_by_dose_group.csv", row.names = FALSE)
